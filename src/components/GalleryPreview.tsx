...
/* ── ALL images from every public folder ────────────────────────────── */
const ALL_IMAGES = [
  /* Safari */
...
export default function GalleryPreview() {
  ...
  return (
    <section id="gallery" className="py-16 sm:py-20 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-xl mx-auto mb-10">
          <div className="inline-flex items-center gap-2 text-[#2e7d32] font-bold text-[11px] tracking-[0.26em] uppercase mb-3">
            <span>📸</span>
            <span>A Glimpse of Kanthalloor</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-black text-[#112918] leading-tight">
            Moments Worth
            <span className="text-[#2e7d32]"> Remembering</span>
          </h2>
          <p className="mt-3 text-gray-500 text-sm leading-relaxed">
            {ALL_IMAGES.length} photos from Safari, Attractions, Nature and Rooms.
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-8 flex-wrap">
          {CATS.map((c) => (
            <button key={c} onClick={() => setFilter(c)}
              ...
              {c}
              <span className={`ml-1.5 text-[10px] ${filter === c ? "text-[#4ade80]" : "text-gray-400"}`}>
                {c === "All" ? ALL_IMAGES.length : ALL_IMAGES.filter((i) => i.cat === c).length}
              </span>
            </button>
          ))}
        </div>

        {/* ── Masonry grid — shows ALL images ── */}
          ...
          {visible.map((img, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <p className="text-white font-extrabold text-xs sm:text-sm leading-tight drop-shadow">
                  {img.title}
                </p>
              </div>
              {/* Zoom icon */}
      ...
      {lightbox !== null && cur && (
        <div
          className="fixed inset-0 z-50 bg-black/93 backdrop-blur-md flex flex-col items-center justify-center p-4"
          onClick={close}
        >
          {/* Close */}
          ...
          <div className="max-w-4xl w-full mx-auto flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            <img
              ...
              alt={cur.title}
              onError={(e) => { (e.target as HTMLImageElement).src = "/jeep/jeep0.png"; }}
              className="max-w-full max-h-[68vh] rounded-2xl shadow-2xl object-contain"
            />
            {/* Caption */}
            <div className="mt-4 text-center">
              ...
              <h3 className="text-white font-extrabold text-base sm:text-lg mt-2">{cur.title}</h3>
            </div>
            {/* Thumbnail strip */}
